...
(() => {
  ...
  const VERSION = '2026-08-26-v4-runtime2';
  const SLOT_BY_ITEM = {
    'Steel Helm':'Head',
    'Leather Hood':'Head',
    'Knight Armor':'Armor',
    'Leather Armor':'Armor',
    'Ruby Ring':'Ring',
    'Adventurer Boots':'Boots',
    'Hunter Cape':'Accessory',
    'Long Sword':'Weapon',
    'Hunter Bow':'Weapon',
    'Arcane Staff':'Weapon',
    'Twin Blades':'Weapon'
  ...
  function ensureContract(){
    ...
    slots.forEach(slot=>{ if(!(slot in s.equipment)) s.equipment[slot]=null; });
    REQUIRED_ITEMS.forEach(item=>{ if(!s.inventory.some(x=>x.id===item.id)) s.inventory.push({...item}); });
    putSave(s); return true;
  }
  function equipV4(id){
    const s=getSave(); if(!s) return window.equip?.(id);
    const item=s.inventory.find(x=>x.id===id); if(!item || item.type==='Consumable') return;
    const slot=item.slot || SLOT_BY_ITEM[item.name] || (item.type==='Weapon'?'Weapon':item.type==='Ring'?'Ring':'Armor');
    s.equipment=s.equipment||{}; ['Head','Armor','Weapon','Accessory','Ring','Boots'].forEach(x=>{if(!(x in s.equipment))s.equipment[x]=null;});
  ...
  function validateSave(){
    const s=getSave(); const result={version:VERSION,valid:true,errors:[]};
    if(!s){result.valid=false;result.errors.push('No save exists');return result;}
    if(!s.hero||!s.inventory||!s.equipment){result.valid=false;result.errors.push('Missing hero/inventory/equipment');}
    const required=['Head','Armor','Weapon','Accessory','Ring','Boots'];
    if(s.equipment && required.some(x=>!(x in s.equipment))){result.valid=false;result.errors.push('Equipment slot contract incomplete');}
    return result;
  }
  function harden(){ ensureContract(); const originalEquip=window.equip; if(originalEquip&&!originalEquip.__v4wrapped){const wrapped=equipV4;wrapped.__v4wrapped=true;window.equip=wrapped;} const originalUnequip=window.unequip; if(originalUnequip&&!originalUnequip.__v4wrapped){const wrapped=unequipV4;wrapped.__v4wrapped=true;window.unequip=wrapped;} window.MMARuntimeLayer={version:VERSION,ensureContract,validateSave,equip:equipV4,unequip:unequipV4}; }
